...
import { MockService, generateChecks } from '@/lib/mock-data';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';
import { formatDistanceToNow, format } from 'date-fns';
import { Trash2, Pause, Play, Settings } from 'lucide-react';
...
export default function ServiceDetailModal({ service, open, onClose, onDelete }: ServiceDetailModalProps) {
  ...
  const checks = useMemo(() => {
    if (!service) return [];
    return generateChecks(service.id, 50);
  }, [service]);
      ...
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div className={`ml-auto ${statusDotClass[service.status]}`} />
          </div>
        </DialogHeader>
        ...
        <Tabs defaultValue="overview" className="mt-4">
          ...
          <TabsContent value="overview" className="space-y-6 mt-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-muted rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-foreground">{service.uptime_percentage}%</p>
                <p className="text-xs text-muted-foreground mt-1">Uptime</p>
              </div>
              <div className="bg-muted rounded-xl p-4 text-center">
                <p className="text-2xl font-bold text-foreground">{service.avg_response_time}ms</p>
          ...
          <TabsContent value="settings" className="mt-4 space-y-4">
            <div className="flex items-center justify-between p-4 border border-border rounded-xl">
              ...
              <Button variant="outline" size="sm" onClick={() => setPaused(!paused)} className="gap-2">
